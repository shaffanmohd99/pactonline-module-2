...
const ChecklistMobile = ({ open, setOpen }) => {
  ...
  if (open) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-overlay z-50">
        <div className="w-full h-full bg-white overflow-hidden max-w-md">
          <div className="w-full h-full pt-4 text-[20px]">
            <div className="flex h-8 border-b border-grayLine pl-4 gap-4">
              <div className="flex h-8  border-b border-grayLine pl-4 gap-4">
        ...
        <div
          onClick={handleClose}
          className="absolute top-4 right-4 cursor-pointer text-darkText hover:text-redPrimary"
        >
          <BsX size={24} />
        </div>
      </div>
